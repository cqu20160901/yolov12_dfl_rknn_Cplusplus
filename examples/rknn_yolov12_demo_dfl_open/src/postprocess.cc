...
/****** yolov12 ****/
GetResultRectyolov12::GetResultRectyolov12()
{
}
...
float GetResultRectyolov12::sigmoid(float x)
{
    return 1 / (1 + fast_exp(-x));
}

int GetResultRectyolov12::GenerateMeshgrid()
{
    int ret = 0;
    if (headNum == 0)
    {
        printf("=== yolov12 Meshgrid  Generate failed! \n");
    }
    ...
    printf("=== yolov12 Meshgrid  Generate success! \n");

    return ret;
}

int GetResultRectyolov12::GetConvDetectionResult(int8_t **pBlob, std::vector<int> &qnt_zp, std::vector<float> &qnt_scale, std::vector<float> &DetectiontRects)
{
    int ret = 0;
    if (meshgrid.empty())
    {
